...
description: |
  Post-implementation backend security review scoped to a feature or flow: injection, broken auth, mass assignment, sensitive data exposure, missing rate limiting, improper authorization, business flow abuse, misconfiguration. Covers OWASP API Security Top 10 2023.
  Use when: a backend feature has been implemented and needs a security pass before PR review or merge. Run after implementation is functionally complete.
  Trigger phrases: "check security", "security review backend", "security audit", "security check after implementation".
  Skip when: task is purely frontend styling, config-only change, or documentation update with no logic.
...
### Insecure Direct Object Reference in File Serving
Files served without verifying the requesting user owns them.
...
## 10. Business Logic & Misconfiguration *(OWASP API6, API8, API9)*
...
- Third-party API responses trusted without validation (API10): type-checking, size-checking, and sanitizing external API data before using it in DB writes or responses
