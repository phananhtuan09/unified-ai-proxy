...
name: api-design
description: |
  REST API design: endpoint naming, HTTP conventions, request/response schema, versioning, pagination, error format.
  Use when: designing new API endpoints, reviewing API contracts, aligning FE/BE on schema, adding versioning strategy, defining error response structure, or any task involving API interface decisions.
  Skip when: implementing existing well-defined API specs with no design decisions needed.
---
...
Use Read to load the file before proceeding when the condition applies:

- **Idempotency keys** (payment, order creation, notifications): Read `.claude/skills/api-design/references/idempotency.md`
- **File uploads**: Read `.claude/skills/api-design/references/file-upload.md`
- **Webhook design**: Read `.claude/skills/api-design/references/webhooks.md`
